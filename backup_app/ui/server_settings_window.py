...
class ServerSettingsWindow(QDialog):
    ...
    def load_existing_settings(self):
        """Загружает сохранённые настройки сервера"""
        settings = load_settings().get("server", {})
        self.server_ip.setText(settings.get("ip", ""))
        self.username.setText(settings.get("username", ""))
        self.password.setText(settings.get("password", ""))
        self.port.setText(str(settings.get("port", "21")))
        server_type = settings.get("type", "FTP")
        index = self.server_type.findText(server_type)
        if index >= 0:
            self.server_type.setCurrentIndex(index)
